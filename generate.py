...
class LookupType:
	...
	def boolType() -> 'LookupType':
		...
		return out
	@staticmethod
	def intType(intType: str, defValue: int) -> 'LookupType':
		out = LookupType()
		out._kind = 'int'
...
class CodeGen:
	...
	def _buffer(self, hint: str, data: list[int]) -> str:
		...
		self._file.write(f'static constexpr {typeString} {name}[{len(data)}] = {{\n\t')
	...
	def _intRangeBinarySearch(self, ranges: list[Range], hint: str, tp: LookupType, inVarName: str, outVarName: str|None) -> str:
		defValue = None
		...
		if (ranges[-1].span() // 0x10000) >= len(ranges):
			defValue = ranges[-1].values
			ranges = ranges[:-1] + [Range(ranges[-1].first, ranges[-1].first + 0x10000 - 1, defValue)]

		# update the ranges to adhere to the size-constraints of 16 bits (larger by one, as values are stored decreased by one)
		ranges: list[Range] = Ranges.limitSize(ranges, 0x10000)
		...
		if len(counts) > 2:
			...
			lookupValueCode = self._intRangeLookup(tempRanges, f'{hint}Value', tp, indexVar, outVarName)
		...
		lookupSizeCode = self._intRangeLookup(self._rangeFromList([r.span() - 1 for r in ranges]), f'{hint}Size', LookupType.intType('size_t', 0), indexVar, lookupSizeOutVar)
		...
		firstBufferName = self._buffer(f'{hint}Start', [r.first for r in ranges])

		# setup the code to perform the binary-search (check input-var greater than size, as size is reduced by one)
		findValue = f'size_t {lookupSizeOutVar} = 0, {indexVar} = {self._file.binSearch(firstBufferName)};\n'
		...
		if outVarName is None:
			...
			findValue += f'else\n\t{lookupValueCode}'
		return findValue
	def _intRangeRawBuffer(self, ranges: list[Range], hint: str, tp: LookupType, inVarName: str, outVarName: str|None) -> str:
		# create the buffer to be written out and return the access string
		bufName = self._buffer(hint, [r.values[0] for r in ranges for _ in range(r.first, r.last + 1)])
		accessString = tp.dynLookup(f'{bufName}[{inVarName}]')
		if outVarName is None:
			return f'return {accessString};\n'
		return f'{outVarName} = {accessString};\n'
	def _intRangeCode(self, ranges: list[Range], hint: str, tp: LookupType, inVarName: str, outVarName: str|None) -> str:
		...
		for cluster in clusters:
			...
			clusterAccess.append(self._intRangeRawBuffer(dropped, f'{hint}Cluster', tp, clusterInVarName, outVarName))
		...
		while True:
			...
			while index < len(actual):
				...
				if StrHelp.multiLines(selectedValue):
					...
					lookupCode += '\n' + StrHelp.indent(selectedValue)
				index += count
	def _intRangeLookup(self, ranges: list[Range], hint: str, tp: LookupType, inVarName: str, outVarName: str|None) -> str:
		...
		if len(ranges) < 40 or (rangeCoveredChars <= 1.75 * actualRangeChars and rangeCoveredChars <= 3.0 * len(ranges)):
			return self._intRangeCode(ranges, hint, tp, inVarName, outVarName)
		...
		if self._inBinSearch == 0 or CodeGen.NestedBinSearch:
			self._inBinSearch += 1
			out = self._intRangeBinarySearch(ranges, hint, tp, inVarName, outVarName)
			self._inBinSearch -= 1
			return out
		return self._intRangeRawBuffer(ranges, hint, tp, inVarName, outVarName)

	def addEnum(self, enum: LookupType) -> None:
		# extract the enum-type to be used
		enumType = 'uint8_t'
		if len(enum.enumValues()) > 2**16:
			enumType = 'uint32_t'
		elif len(enum.enumValues()) > 2**8:
			enumType = 'uint16_t'

		# write the enum out
		self._file.write(f'enum class {enum.typeName(True)} : {enumType} {{\n\t')
		self._file.writeln(",\n\t".join(enum.enumValues()))
		self._file.writeln('};')
	def intLookupFunction(self, fnName: str, lookupType: LookupType, ranges: list[Range]) -> None:
		...
		asciiCode = ('' if len(asciiRanges) == 0 else self._intRangeLookup(asciiRanges, 'Ascii', lookupType, 'cp', None))
		lowerCode = ('' if len(lowerRanges) == 0 else self._intRangeLookup(lowerRanges, 'Low', lookupType, 'cp', None))
		higherCode = ('' if len(upperRanges) == 0 else self._intRangeLookup(upperRanges, 'High', lookupType, 'cp', None))
		...
		# generate the actual function and construct the conditional body
...
def MakeCodepointQuery():
	...
	with GeneratedFile('generated/unicode-cp-query.h', True) as file:
		...
		_gen.intLookupFunction('TestUnicode', LookupType.boolType(), unicodeRanges)
		...
		_gen.intLookupFunction('TestAssigned', LookupType.boolType(), assignedRanges)
		...
		_gen.intLookupFunction('TestAscii', LookupType.boolType(), asciiRanges)
		...
		_gen.intLookupFunction('TestAlpha', LookupType.boolType(), alphaRanges)
		...
		_gen.intLookupFunction('GetRadix', LookupType.intType('uint8_t', 0xff), radixRanges)
		...
		_gen.intLookupFunction('GetDigit', LookupType.intType('uint8_t', 0xff), digitRanges)
		...
		_gen.intLookupFunction('TestWhiteSpace', LookupType.boolType(), whiteSpaceRanges)
		...
		_gen.intLookupFunction('TestControl', LookupType.boolType(), controlRanges)
		...
		_gen.intLookupFunction('TestLetter', LookupType.boolType(), letterRanges)
		...
		_gen.intLookupFunction('TestAlNum', LookupType.boolType(), alnumRanges)
		...
		_gen.addEnum(_enum)
		_gen.intLookupFunction('GetPrintable', _enum, printableRanges)
		...
		_gen.addEnum(_enum)
		_gen.intLookupFunction('GetCase', _enum, caseRanges)
		...
		_gen.addEnum(_enum)
		_gen.intLookupFunction('GetCategory', _enum, categoryRanges)
...
def MakeCodepointMaps():
	...
	upperRanges = FilterProperties(unicodeData, 13, lambda fs: int(fs[12], 16) if fs[12] != '' else None)
